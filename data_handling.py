...
def import_raw_material_manual(global_data, status_var):
    ...
    try:
        ...
        # 5. 更新界面和提示
        update_overview_table(global_data["tree_overview"], "毛坯基础信息", len(filtered_df), raw_material_path)
...
def download_and_process_daily_order():
    ...
    if os.path.exists(save_dir):
        ...
        for file_name in os.listdir(save_dir):
            # 使用正则表达式提取文件名中的日期
            match = re.match(r'(\d{8})_采购订单\.xlsx', file_name)
            if match:
                file_path = os.path.join(save_dir, file_name)
                # 获取文件的修改时间
                mod_time = os.path.getmtime(file_path)
                file_date = match.group(1)
                valid_files.append((mod_time, file_date, file_path))
        
        # 如果有符合条件的文件，按修改时间排序，取最新的一个
        if valid_files:
            # 按修改时间降序排序
            valid_files.sort(key=lambda x: x[0], reverse=True)
            # 获取最新的文件
            latest_mod_time, latest_file_date, latest_file_path = valid_files[0]
            
            print(f"找到最新的每日订单文件: {latest_file_path}")
            print(f"文件日期: {latest_file_date}, 当前日期: {today}")
            
            # 检查最新文件是否为当天的
            if latest_file_date == today:
                print("最新文件为当天的，直接导入")
                try:
                    # 尝试读取文件
                ...
                print("最新文件不是当天的，需要进行下载")
    
    # 没有找到当前日期的文件，或者读取失败，尝试下载
    print("没有找到当前日期的每日订单文件，开始下载...")
                    ...
                    if isinstance(json_data, dict):
                        ...
                        if json_data.get('code') == 401 or 'token失效' in json_data.get('msg', '') or '重新登录' in json_data.get('msg', ''):
                            ...
                            if attempt < max_attempts - 1:
                                # 导入open_embedded_login函数
                                from main import open_embedded_login
                                # 打开浏览器进行登录
                                login_success = open_embedded_login()
                                if login_success:
                                    # 获取新的token
                                    new_token = get_token()
                                    if new_token:
...
def import_daily_order_manual(global_data, status_var):
    ...
    try:
        ...
        update_overview_table(global_data["tree_overview"], "每日订单", len(global_data["daily_order_df"]), daily_order_path)
...
def import_weekly_order_manual(global_data, status_var):
    ...
    try:
        ...
        update_overview_table(global_data["tree_overview"], "每周订单", len(global_data["weekly_order_df"]), weekly_order_path)
...
def import_finished_stock_manual(global_data, status_var):
    ...
    try:
        ...
        update_overview_table(global_data["tree_overview"], "当日成品库存", unique_material_count, finished_stock_path)
...
def import_raw_stock_manual(global_data, status_var):
    ...
    try:
        ...
        # 8. 更新概览表格和状态提示
        update_overview_table(global_data["tree_overview"], "当日毛坯库存", unique_material_count, raw_stock_path)
...
def import_yearly_estimate_manual(global_data, status_var):
    ...
    try:
        ...
        # 更新概览表格
        update_overview_table(global_data["tree_overview"], "年预计量", len(yearly_df), yearly_estimate_path)
...
def quick_import_all_last_files(global_data, status_var):
    ...
    # 检查token是否有效
    status_var.set("正在检查token有效性...")
    if not check_token_validity():
        # token无效，打开嵌入式登录页面获取新token
        status_var.set("登录令牌失效，正在打开登录页面...")
        # 显示提示弹窗
        messagebox.showinfo("提示", "登录令牌失效，请在稍后打开的浏览器中重新登录")
        try:
            # 导入open_embedded_login函数
            from main import open_embedded_login
            login_success = open_embedded_login()
            if not login_success:
                messagebox.showerror("错误", "登录失败，无法获取有效的token！")
                status_var.set("登录失败，导入操作取消")
                return
        except Exception as e:
            messagebox.showerror("错误", f"打开登录页面失败：{str(e)}")
            status_var.set("打开登录页面失败，导入操作取消")
            return
            ...
            if data_type in last_import_paths:
                ...
                try:
                    if data_type == "raw_material":
                        ...
                        global_data["raw_material_df"] = filtered_df
                        update_overview_table(global_data["tree_overview"], "毛坯基础信息", len(filtered_df), path)
                        imported_count += 1
                        ...
                        if "喷涂订单整理" in excel_file.sheet_names:
                            ...
                            update_overview_table(global_data["tree_overview"], "每周订单", len(global_data["weekly_order_df"]), path)
                            imported_count += 1
                        else:
                        ...
                        for sheet_name in excel_file.sheet_names:
                            ...
                            for _, row in sheet_df.iterrows():
                                ...
                                # 提取关键字段
                                order_data = {
                                    '采购日期': str(row.iloc[0]).strip() if pd.notna(row.iloc[0]) else '',   # A列：采购日期
                                    '下单日期': row.iloc[0],                 # A列：采购日期
                                    '交货日期': row.iloc[4],                 # E列：预计交货日期
                                    '毛坯编码': str(row.iloc[1]).strip() if pd.notna(row.iloc[1]) else '',     # B列：物料列表
                                    '毛坯名称': str(row.iloc[2]).strip() if pd.notna(row.iloc[2]) else '',     # C列：物料名称
                                    '采购数量': row.iloc[3],                 # D列：采购数量
                                    '成品编码': str(row.iloc[4]).strip() if pd.notna(row.iloc[4]) else '',     # E列：预计交货日期
                                    '实际交货时间': row.iloc[5],             # F列：实际交货时间
                                    '实际交货数量': row.iloc[6]              # G列：实际交货数量
                                }
                                all_data.append(order_data)
                                total_valid_rows += 1
                        
                        if all_data:
                            global_data["ordered_df"] = pd.DataFrame(all_data)
                            update_overview_table(global_data["tree_overview"], "已下单文件", len(global_data["ordered_df"]), path)
                            imported_count += 1
                        else:
                            messagebox.showerror("错误", f"{display_name}表中无有效数据！")
                            status_var.set(f"{display_name}导入失败 - 无有效数据")
                            failed_count += 1
                            failed_types.append(display_name)
                            continue
                        ...
                        summary_df = stock_df.groupby("物料编码", as_index=False).agg({"库存数量": "sum"})
                        global_data["finished_stock_df"] = summary_df
                        update_overview_table(global_data["tree_overview"], "当日成品库存", len(summary_df), path)
                        imported_count += 1
                        ...
                        summary_df = raw_df.groupby("物料编码", as_index=False).agg({"库存数量": "sum"})
                        global_data["raw_stock_df"] = summary_df
                        update_overview_table(global_data["tree_overview"], "当日毛坯库存", len(summary_df), path)
                        imported_count += 1
                        ...
                        try:
                            df_str = pd.read_excel(excel_file, sheet_name=sheet_names[0], dtype=str)
                            global_data["yearly_estimate_df"] = df_str
                            update_overview_table(global_data["tree_overview"], "年预计量", len(df_str), path)
                            imported_count += 1
                        except Exception as inner_e:
                            messagebox.showerror("错误", f"{display_name}导入失败：{str(inner_e)}")
                            status_var.set(f"{display_name}导入失败：{str(inner_e)}")
                            failed_count += 1
                            failed_types.append(display_name)
                            continue
                    
                    elif data_type == "daily_order":
                        # 使用智能逻辑导入每日订单：检查最新文件是否为当天的
                        try:
                            status_var.set(f"正在智能处理：{display_name}...")
                            # 调用download_and_process_daily_order函数，该函数会检查最新文件是否为当天的
                            daily_df, file_path = download_and_process_daily_order()
                            if daily_df is not None and not daily_df.empty:
                                global_data["daily_order_df"] = daily_df
                                # 更新历史路径为最新的文件路径
                                last_import_paths["daily_order"] = file_path
                                global_data["last_import_paths"] = last_import_paths
                                update_overview_table(global_data["tree_overview"], "每日订单", len(daily_df), file_path)
                                imported_count += 1
                            else:
                                messagebox.showwarning("警告", f"{display_name}表为空或下载失败！")
                                status_var.set(f"{display_name}导入失败 - 表格为空或下载失败")
                                failed_count += 1
                                failed_types.append(display_name)
                        except Exception as inner_e:
                            messagebox.showerror("错误", f"{display_name}导入失败：{str(inner_e)}")
...
def import_ordered_file_manual(global_data, status_var):
    ...
    try:
        ...
        # 获取Excel文件的所有工作表
        excel_file = pd.ExcelFile(ordered_file_path)
        sheet_names = excel_file.sheet_names
        ...
        for sheet in sheet_names:
            ...
            total_valid_rows += sheet_valid_rows
            
            # print(f"工作表{sheet}有效行数:", sheet_valid_rows)
            # print(f"工作表{sheet}跳过行数:", sheet_skipped_rows)
        ...
        if all_order_data:
            # 构建DataFrame
            ordered_df = pd.DataFrame(all_order_data)
            global_data["ordered_df"] = ordered_df
            
            # 更新概览表格
            update_overview_table(global_data["tree_overview"], "已下单文件", len(ordered_df), ordered_file_path)
            
            # 提示成功信息
            messagebox.showinfo(
                "成功",
                f"已下单文件导入成功！\n"
                f"有效数据：{total_valid_rows} 行"
            )
            status_var.set(f"已下单文件导入成功 - 共{total_valid_rows}条有效数据")
        else:
            messagebox.showerror("错误", "已下单文件中无有效数据！")
            status_var.set("已下单文件导入失败 - 无有效数据")
...
# ========== 新增：更新概览表格函数 ==========
def update_overview_table(tree, data_type, count, filename=""):
    """
    更新概览表格中的数据数量
    tree: 概览表格的Treeview组件
    data_type: 数据类型名称
    count: 数据数量
    filename: 导入的文件名
    """
    ...
    # 遍历表格中的所有行，找到与data_type匹配的行
    updated = False
    for item in tree.get_children():
        values = tree.item(item, 'values')
        if values and values[0] == data_type:
            # 更新该行的数据数量、文件名和最后更新时间
            tree.item(item, values=(data_type, display_filename, count, current_time))
            updated = True
            break
    
    # 如果没有找到匹配的行，插入一行新数据
    if not updated:
        tree.insert('', 'end', values=(data_type, display_filename, count, current_time))

# ========== 采购分析相关函数 ==========
def analyze_procurement_status(global_data, status_var):
    """
    采购分析 - 统计未到货数量和已到货数量
    核心逻辑：
    1. 从每周订单中提取采购订单数据（过滤订单状态为"0"）
    2. 计算每个订单的总数量、未到货数量和已到货数量
    3. 按供应商和订单类型分组统计
    """
    try:
        # 检查必要的数据是否存在
        if "weekly_order_df" not in global_data or global_data["weekly_order_df"].empty:
            messagebox.showerror("错误", "请先导入每周订单数据！")
            status_var.set("采购分析失败 - 缺少每周订单数据")
            return
        
        weekly_df = global_data["weekly_order_df"]
        
        # 检查必要的列是否存在
        required_columns = ['订单状态', '供应商', '订单类型', '订单数量', '已到货数量']
        if not all(col in weekly_df.columns for col in required_columns):
            messagebox.showerror("错误", "每周订单数据缺少必要的列！")
            status_var.set("采购分析失败 - 每周订单数据列不完整")
            return
        
        # 过滤订单状态为"0"的采购订单
        procurement_df = weekly_df[weekly_df['订单状态'] == '0'].copy()
        
        if procurement_df.empty:
            messagebox.showinfo("提示", "当前没有订单状态为'0'的采购订单")
            status_var.set("采购分析完成 - 无符合条件的采购订单")
            return
        
        # 计算未到货数量
        procurement_df['未到货数量'] = procurement_df['订单数量'] - procurement_df['已到货数量']
        ...
        summary_df = procurement_df.groupby(group_cols, as_index=False).agg(agg_dict)
        summary_df.rename(columns={'订单状态': '订单数'}, inplace=True)
        
        # 保存分析结果到全局数据
        global_data["procurement_analysis_df"] = summary_df
        
        # 计算总体统计
        total_orders = summary_df['订单数'].sum()
        total_quantity = summary_df['订单数量'].sum()
        total_received = summary_df['已到货数量'].sum()
        total_pending = summary_df['未到货数量'].sum()
        ...
        report = (
            ...
            f"==================================\n" +
            f"总体统计：\n" +
            f"- 订单总数：{total_orders} 个\n" +
            f"- 总订单数量：{total_quantity:.0f} 件\n" +
            f"- 已到货数量：{total_received:.0f} 件\n" +
            f"- 未到货数量：{total_pending:.0f} 件\n" +
            f"==================================\n" +
            f"按供应商和订单类型分组统计：\n" +
            f"==================================\n"
        )
        ...
        for _, row in summary_df.iterrows():
            report += (
                ...
                f"订单类型：{row['订单类型']}\n" +
                f"- 订单数：{row['订单数']} 个\n" +
                f"- 订单数量：{row['订单数量']:.0f} 件\n" +
                f"- 已到货数量：{row['已到货数量']:.0f} 件\n" +
                f"- 未到货数量：{row['未到货数量']:.0f} 件\n" +
                f"----------------------------------\n"
            )
        ...
        status_var.set(f"采购分析完成 - 共分析{total_orders}个订单")
        
    except Exception as e:
        messagebox.showerror("错误", f"采购分析失败：{str(e)}")
        status_var.set(f"采购分析失败：{str(e)}")

# ========== 订单追踪列表函数 ==========
def generate_order_tracking_list(global_data, status_var):
    """
    生成订单追踪列表
    核心逻辑：
    1. 从已下单文件中提取订单数据
    ...
    try:
        ...
        required_data = [
            ("ordered_df", "已下单文件"),
            ("weekly_order_df", "每周订单"),
            ("raw_material_df", "毛坯基础信息"),
            ("finished_stock_df", "当日成品库存")
        ]
        
        for data_key, data_name in required_data:
            if data_key not in global_data or global_data[data_key].empty:
                messagebox.showerror("错误", f"请先导入{data_name}数据！")
                status_var.set(f"订单追踪列表生成失败 - 缺少{data_name}数据")
                return
        
        ordered_df = global_data["ordered_df"]
        weekly_df = global_data["weekly_order_df"]
        raw_material_df = global_data["raw_material_df"]
        finished_stock_df = global_data["finished_stock_df"]
        
        # 检查已下单文件的必要列
        ordered_required_columns = ['采购日期', '下单日期', '交货日期', '毛坯编码', '成品编码', '实际交货时间', '实际交货数量']
        if not all(col in ordered_df.columns for col in ordered_required_columns):
            messagebox.showerror("错误", "已下单文件缺少必要的列！")
            status_var.set("订单追踪列表生成失败 - 已下单文件列不完整")
            return
        
        # 检查每周订单的必要列
        weekly_required_columns = ['订单编号', '供应商', '订单类型', '订单数量', '已到货数量']
        if not all(col in weekly_df.columns for col in weekly_required_columns):
            messagebox.showerror("错误", "每周订单文件缺少必要的列！")
            status_var.set("订单追踪列表生成失败 - 每周订单文件列不完整")
            return
        
        # 关联并整合数据，生成追踪列表
        # 这里是简化的实现，实际逻辑可能更复杂
        tracking_list = ordered_df.copy()
        
        # 添加过滤条件：只保留F列（实际交货时间）和G列（实际交货数量）都为空的记录
        tracking_list = tracking_list[
            (tracking_list['实际交货时间'].isna() | (tracking_list['实际交货时间'].astype(str).str.strip() == '')) &
            (tracking_list['实际交货数量'].isna() | (tracking_list['实际交货数量'].astype(str).str.strip() == ''))
        ]
        
        # 添加状态信息
        tracking_list['订单状态'] = '已下单'
        
        status_var.set("订单追踪列表生成成功")
        messagebox.showinfo("成功", "订单追踪列表生成成功！")
    except Exception as e:
        messagebox.showerror("错误", f"订单追踪列表生成失败：{str(e)}")
        status_var.set(f"订单追踪列表生成失败：{str(e)}")

# ========== 分析报告保存和加载函数 ==========
def save_analysis_report(report_data, file_path):
    """
    保存分析报告
    """
    try:
...
def load_history_reports(tree_history, reports_dir):
    """
    加载历史报告到树状结构
    """
    try:
        ...
        for file_name in report_files:
            # 提取生成时间（假设文件名包含时间戳）
            generated_time = file_name
            tree_history.insert("", "end", values=(file_name, generated_time, "查看"))
    except Exception:
        pass