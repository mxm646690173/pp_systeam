# ui_analysis_pandastable.py - 使用pandastable的高性能分析结果展示模块
...
def build_analysis_tab(parent, global_data, path_config):
    ...
    if PANDASTABLE_AVAILABLE:
        # 创建中间容器用于grid布局
        table_container = ttk.Frame(frame_table)
        ...
        # 创建Table实例
        table = Table(table_container, dataframe=df, showtoolbar=False, showstatusbar=False)
        table.show()
        ...
        widgets["table_analysis"] = table
        widgets["tree_analysis"] = table  # 向后兼容
        ...
        def handle_double_click(event):
            try:
                ...
                if row is not None and row >= 0:
                    df = table.model.df
                    if row < len(df):
                        blank_code = df.iloc[row].get("毛坯编码", "")
                        if blank_code and blank_code != "-":
                            show_blank_code_orders(blank_code, global_data)
            except Exception as e:
                pass
        
        table.bind("<Double-Button-1>", handle_double_click)
    else:
        # 回退到Treeview
        ttk.Label(frame_table, text="pandastable未安装，使用Treeview").pack()
...
def update_analysis_table(widgets, global_data, filter_type="all"):
    """
    更新分析结果表格数据（使用pandastable）
    """
    if not PANDASTABLE_AVAILABLE:
        return
    
    table = widgets["table_analysis"]
    
    # 获取分析数据
    analysis_df = global_data.get("analysis_show_df")
    if analysis_df is None or analysis_df.empty:
        empty_df = pd.DataFrame(columns=widgets["table_columns"])
        table.updateModel(empty_df)
        table.redraw()
        return
    ...
    for idx in range(len(display_df)):
        ...
        for table_col in widgets["table_columns"]:
            ...
            if pd.isna(val) or val == "":
                ...
                val = str(val)
            row_values[table_col] = val
        table_data.append(row_values)
    
    # 创建显示DataFrame
    display_table_df = pd.DataFrame(table_data)
    
    # 更新表格
    try:
        table.updateModel(display_table_df)
        table.redraw()
    except Exception as e:
        print(f"更新表格时出错: {e}")
        pass
